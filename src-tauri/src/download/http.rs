...
//!
//! Used as a fast path before (or alongside) the WebView-based SEGGER flow.
...
pub async fn download_to_path(
    ...
    let client = reqwest::Client::builder()
        .user_agent("probe-configurator/1.0 (tauri; reqwest)")
    ...
    if !resp.status().is_success() {
        return Err(AppError::DownloadFailed(format!("HTTP {}", resp.status())));
    }

    // If SEGGER responds with an HTML page (license gate), bail out so caller can use WebView.
